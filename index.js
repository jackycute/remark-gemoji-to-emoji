...
function transformer(tree) {
 ...
 /* :-1: */ thumbsdown: /:\-1:/g
        ...
        node.value = node.value.replace(emoticons.thumbsup, gemoji.name['+1'].emoji);
        node.value = node.value.replace(emoticons.thumbsdown, gemoji.name['-1'].emoji);
        ...
        while ((match = emoticons.named.exec(node.value)) !== null) {
            if (match.index === emoticons.named.lastIndex) {
                emoticons.named.lastIndex++;
            }
        ...
        for (var i = 0, l = matches.length; i < l; i++) {
            match = matches[i];
            node.value = node.value.substr(0, match.index + bias) + match.emoji + node.value.substr(match.index + match.str.length + bias);
